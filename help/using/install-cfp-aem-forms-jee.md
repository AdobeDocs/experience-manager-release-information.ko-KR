...
1. `adobe.<serviceName>.<operationName>.timeout`: 키에 언급된 특정 서비스 작업(&lt;serviceName>.&lt;operationName>)에 대한 타임아웃만 덮어씁니다. 이 값이 작업 수준에서 설정된 경우, 이 명령을 사용하면 지정된 서비스가 애플리케이션 수준 또는 서비스 수준에서 설정된 경우에만 해당 서비스에 대한 시간 제한 값을 덮어씁니다.
