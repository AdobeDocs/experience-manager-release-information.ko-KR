...
* 클라우드 서비스를 사용하여 Google `reCaptcha`를 구성할 때 외부 연결에 org.apache.http.proxyconfigurator 구성이 필요한 환경에서는 POST 호출이 프록시를 통과하는 것 같지 않습니다. NPR-20454
...
* AEM 6.3.3.0은 CFP를 위한 필수 구성 요소입니다. AEM 설치를 AEM 6.3으로 업그레이드하는 방법에 대한 세부 지침은 [업그레이드 설명서](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)를 참조하십시오.
...
기본값은 1입니다. 값은 CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL 속성의 주석을 해제하고 필요한 값(페이지 속성 데이터가 저장될 때까지 페이지의 `/jcr:content` 아래의 최대 깊이)을 할당하여 constants.js(`/libs/cq/ui/widgets/source/constants.js`) 파일을 오버레이해서 재정의합니다.
